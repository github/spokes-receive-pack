...
func TestPipelineSingleCommandWithStdout(t *testing.T) {
	...
	p := pipe.New(dir, pipe.WithStdout(stdout))
	p.Add(pipe.Command("echo", "hello world"))
...
func TestPipelineReadFromSlowly(t *testing.T) {
	...
	p := pipe.New(dir, pipe.WithStdout(w))
	p.Add(pipe.Command("echo", "hello world"))
...
func TestPipelineSingleGitCommandWithOutput(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
...
func TestPipelineStderr(t *testing.T) {
	...
	buf := &bytes.Buffer{}
	logger := log.New(log.InfoLevel, &log.Logfmt{Sink: &log.SyncedWriter{Writer: buf}})

	p := pipe.New(dir, pipe.WithLogger(logger))
	...
	assert.Contains(t, buf.String(), "doesnotexist")
}
